...
type DNSRecord = Cloudflare.DNS.Records.ARecord | Cloudflare.DNS.Records.AAAARecord;
...
export class DDNS_Service {
  ...
  async processUpdate(dnsRecordName: string): Promise<DNSRecord[]> {
  ...
  async fetchCurrentIp(type: string): Promise<string> {
    let queryUrl = '';
    if (type === 'A') {
